...
class WinningRequest_model extends CI_Model{
	
	public function getAllWinningRequests($data)
	{
		...
		if($status=='pending')
		{
			$software_where  	.= ' AND c.room_winner_status = "PROCESSING" ';
			$pageArr = array('room_winner_status'=>'PROCESSING');
		}elseif($status=='transfered')
		{
			$software_where  	.= ' AND c.room_winner_status = "DELIVERED" ';
			$pageArr = array('room_winner_status'=>'DELIVERED');
		}
		
		if($data['room_name'] != ""){
			$software_where .= ' AND r.room_name Like "%'.$data['room_name'].'%"  ';
		}	
		if($data['room_category'] != ""){
			$software_where .= ' AND r.category_id = "'.$data['room_category'].'" ';
		}
		if($data['room_status'] != ""){
	   ...
	   $data['where_coloums'] 	    = array('serial_number','category_name','room_name','full_name');
       $data['select_order_colum']  = array('serial_number','room_id','full_name','emailid','room_name','category_name','room_image','type','credit_to_amount','credit_to_app_balance');
		...
		$query = "SELECT SQL_CALC_FOUND_ROWS @a:=@a+1 as serial_number,c.*,ct.category_name,u.full_name,u.emailid FROM "."
		room_winners c
		...
		$total_records = $this->list_rooms_count($pageArr);
		if($where!=""){
		...
		foreach($result as $aRow )
		{
			...
			for ( $i=0; $i<count($data['select_order_colum']) ; $i++ )
			{
				...
				}else
				{
					...
					$action .= 
						'<span>
							<a href="'. base_url().'admin/transfer-fund/'.base64_encode($aRow['room_winner_id']).'" class="btn btn-primary button_icon" title="View Details/Transfer">View Details</a>
						</span>
						';
					$action .= '</div>';
	...
	public function list_rooms_count($pageArr) {
		
		$this->db->select('room_winner_id');
		$this->db->from('room_winners');
		$this->db->where('status','1');
		$this->db->where('room_winner_status','PROCESSING');
		$this->db->where($pageArr);
	...
	public function get_winner_request_details($room_winner_id){
		
		$this->db->select('rw.*, ct.category_name, u.full_name, u.emailid, ua.bank_name, ua.card_number, ua.card_holder_name, ua.security_code');
		$this->db->from('room_winners rw');
		$this->db->join('usermaster u','u.id=rw.user_id','left');
		$this->db->join('user_account_details ua','ua.account_id=rw.account_id','left');
		$this->db->join('categories ct','rw.category_id=ct.category_id','left');
		$this->db->where('room_winner_id',$room_winner_id);
		$result = $this->db->get();
		if($result->num_rows() > 0) {
	...
	public function complete_winner_request($room_winner_id)
	{
		$this->load->library('user_agent');
		$referrer = $this->agent->referrer();	

		$this->db->select('rw.*, ct.category_name, u.full_name, u.emailid, u.user_tokens as available_amount, ua.bank_name, ua.card_number, ua.card_holder_name, ua.security_code');
		$this->db->from('room_winners rw');
		$this->db->join('usermaster u','u.id=rw.user_id','left');
		$this->db->join('user_account_details ua','ua.account_id=rw.account_id','left');
		$this->db->join('categories ct','rw.category_id=ct.category_id','left');
		...
		if($result->num_rows() > 0) {
			$details = $result->row_array();
			if($details['room_winner_status']=='DELIVERED')
			{
				$this->session->set_flashdata('flashError','This request is already completed!');
			}else{
				$updated_user_tokens = $details['available_amount']+$details['credit_to_app_balance'];
				...
				$token_details = array(
					...
					'tokens'			=>	$details['credit_to_app_balance'],
					'type'				=>	'PAYMENT_TRANSFERED',
				...
				$data = array('room_winner_status'=>'DELIVERED','room_prize_deliver_date'=>date('Y-m-d H:i:s'),'modified_on'=>date('Y-m-d H:i:s'));
				$condition = array('room_winner_id'=>$room_winner_id);
				$saveData = $this->commonmodel->_update('room_winners',$data,$condition);
				
				    $tempArr = array();
					$tempArr['room_id'] = $details['room_id'];
					$tempArr['user_id'] = $details['user_id'];
					$tempArr['action'] 	= 'PAYMENT_TRANSFERED';
					$tempArr['created_on'] = date('Y-m-d H:i:s');
					$tempArr['modified_on'] = date('Y-m-d H:i:s');
					$this->db->insert('user_notifications',$tempArr);
				    
				    $to = $details['emailid'];
					$user_name = $details['full_name'];
					$subject = SITE_NAME." Payment transfered";
					$message =	'Hello '.$user_name.'<br/>';
					$message .= 'Your winning request payment has been transfered to your account please check it. <br/> <br/>';
					...
					$user_id = $details['user_id'];
            		$message = 'Your winning request payment has been transfered to your account please check it.';
				...
				$this->session->set_flashdata('flashSuccess','Fund transfer request completed successfully!');
			}
		}else{
			$this->session->set_flashdata('flashError','No request found in system!');
		}
		redirect($referrer);
	}
	...
	public function addWinningRequest($post,$is_copy='',$room_id=''){
		if($post){
			//pr($post,1);
			$data = array();
	...
	public function updateWinningRequest($post,$room_id=""){
		//echo $room_id;
		if($room_id!=""){
			$data = array();
	...
	public function completeEvent($room_id,$post)
	{
		...
		$this->commonmodel->_update('rooms',array('event_completed'=>'1','modified_on'=>date('Y-m-d H:i:s')),array('room_id'=>$room_id));
		return true;
	}
	...
	public function getRoomDetailsById($room_id=""){
		//pr($room_id,1);
		if($room_id!=""){
			$this->db->select('ss.*,c.category_name,COUNT(room_winner_id) as winners');
