...
class Performers_month_model extends CI_Model{
	...
	public function getAllPerformers($data)
	{
       ...
       $data['select_order_colum']  = array('serial_number','name','month','year','rank');
		$data['table_name']   		= "team_of_month";
		$data['indexColumn']  		= "team_of_month_id";
		$limit = '';
		...
		if(isset($data['post']['iSortCol_0']))
		{
			...
			{
				$order_by = "ORDER BY team_of_month_id DESC";
			}
		}
		$where = "";
		...
		foreach($result as $aRow )
		{
			...
			for ( $i=0; $i<count($data['select_order_colum']) ; $i++ )
			{
				...
				{
				   if($data['select_order_colum'][$i] == "performer_month_image")
				   {
				   		if($aRow['performer_month_image']!='' && is_file(UPLOAD_PHYSICAL_PATH.'performer_months_month/'.$aRow['performer_month_image']))
				   		{
				   			$performer_month_image = '<img src="'.UPLOAD_URL.'performer_months_month/'.$aRow['performer_month_image'].'" width="150" />';
				   		}else{
				   			$performer_month_image = '<img src="'.UPLOAD_URL.'performer_months_month/default_performer_month_image.jpg" width="150" />';
				   		}
					   $row[] = $performer_month_image;
				   }
				...
				}else
				{
					...
					$action .= 
						'<span>
							<a href="'. base_url().'admin/edit-performer-month/'.base64_encode($aRow['team_of_month_id']).'" class="btn btn-color button_icon" title="View"><i class="mdi mdi-pencil-box-outline"></i></a>
						</span>';				
					$action .= 
						'<span>
							<a href="'. base_url().'admin/delete-performer-month/'.base64_encode($aRow['team_of_month_id']).'" class="btn btn-color button_icon" title="Delete Item" onclick="return confirm_delete();"><i class="mdi mdi-close"></i></a>
						</span>';
					$action .= '</div>';
	...
	public function list_item_count() {
		...
		$this->db->select('team_of_month_id');
	...
	public function getPerformerDetails($team_of_month_id) {
		
		$this->db->select('*');
		$this->db->from('team_of_month');
		$this->db->where('team_of_month_id',$team_of_month_id);
		$result = $this->db->get();
		if($result->num_rows() > 0) {
	...
	public function addPerformer($post){
		if($post)
		{
			...
			if($this->commonmodel->_insert('team_of_month', $data)){
				return true;
			}else{
				return false;
			}
