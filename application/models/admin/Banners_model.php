...
class Banners_model extends CI_Model{
	...
	public function getAllBanners($data)
	{
       ...
       $data['select_order_colum']  = array('serial_number','heading','heading_gr','heading_tr','sub_heading','sub_heading_gr','sub_heading_tr','banner_image');
		$data['table_name']   		= "tbl_banners";
			...
			for ( $i=0; $i<count($data['select_order_colum']) ; $i++ )
			{
				if ($data['select_order_colum'][$i] != ' ' )
				{
				   if($data['select_order_colum'][$i] == "banner_image")
				   {
				   		...
				   		{
				   			$banner_image = '<img src="'.UPLOAD_URL.'banners/'.$aRow['banner_image'].'" width="100" />';
				   		}else{
				   			$banner_image = '<img src="'.UPLOAD_URL.'banners/default_banner_image.jpg" width="100" />';
				   		}
					   $row[] = $banner_image;
				   }else{					   
	...
	public function getMiddleBnnerDetails() {
