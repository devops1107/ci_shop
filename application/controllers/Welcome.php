...
class Welcome extends CI_Controller {
	...
	public function verifyEmail()
	{
		...
		//echo  $email_varification_code; die;
		
		$condition = array('emailid'=>$emailid,'status'=>'1','delete_status'=>'0');
		//pr($condition,1);
		$details = $this->db->select('*')->from('usermaster')->where($condition)->get();
		if($details->num_rows()>0)
		{
			...
			if(hash('sha256', $emailDetails['email_varification_code'])==$email_varification_code)
			{
				...
				if($emailDetails['email_varification_status']=='0')
				{
					...
					$update_data = array('email_varification_status' =>'1','user_tokens'=>$user_tokens,'modify'=>date("Y-m-d H:i:s"));
					$this->db->where('emailid',$emailid);
					$result = $this->db->update('usermaster',$update_data);
					
					$tokendata = array('user_id'=>$emailDetails['id'],'tokens'=>$signup_reward_details['signup_reward_tokens'],'type'=>'SIGNUP_REWARD','is_claim_allowed'=>'0','is_claimed_status'=>'0','created_on'=>date('Y-m-d H:i:s'));
					$saveData = $this->db->insert('token_purchases',$tokendata);
					if($emailDetails['refferer_code']!="" && $emailDetails['refferer_user_id']!=""){
						$this->db->select('id,user_tokens');
						$this->db->from('usermaster');
						$this->db->where('status','1');
						$this->db->where('delete_status','0');
						$this->db->where('id',$emailDetails['refferer_user_id']);
						$refferquery = $this->db->get();
						$reffererUserData = $refferquery->row_array();
						$updateresult = $this->db->update('usermaster',array('user_tokens'=>$reffererUserData['user_tokens']+$signup_reward_details['refferal_reward_tokens'],'modify'=>date('Y-m-d H:i:s')),array('id'=>$emailDetails['refferer_user_id']));
						
						$tokenRefferdata = array('user_id'=>$reffererUserData['id'],'tokens'=>$signup_reward_details['refferal_reward_tokens'],'is_claim_allowed'=>'0','type'=>'REFER_EARN','created_on'=>date('Y-m-d H:i:s'));
						$result1 = $this->db->insert('token_purchases',$tokenRefferdata);
						
						$tokenRefferdata1 = array('user_id'=>$emailDetails['id'],'tokens'=>$signup_reward_details['refferal_reward_tokens'],'is_claim_allowed'=>'0','type'=>'REFER_EARN','created_on'=>date('Y-m-d H:i:s'));
						$result2 = $this->db->insert('token_purchases',$tokenRefferdata1);
					}
				   ...
				   //echo 'failed'; die;
					$msg = 'user_login_error, You are already verified.';
					echo $msg; die;
					
				}
			}else{
			   // echo 'expired'; die;
			   $msg = 'user_verified_error, Your link has expired ! Please try again';
					echo $msg; die;
			/*	$this->session->set_flashdata('user_login_error, Token mismatch.Please hit correct url.');
				redirect('home');*/
			}
		}else{
			$msg = 'user_register_error, You are not register with us.Please register first.';
			echo $msg; die;
			...
			if($emailDetails['forgot_verify_code']!="")
			{
				if($this->input->post())
				{
					if($this->input->post('password') != $this->input->post('confirmpassword'))
					{
						...
						$this->commonmodel->_update('usermaster',$update_data,array('user_id'=> $emailDetails['user_id']));
